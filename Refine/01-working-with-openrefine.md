...
````
  - Scroll to Country.  Look at the fourth row. Edit "United States of America" to "UNITED STATES" and apply to all matching cells. 
  - Create a text facet of Country. Edit "US" to match "UNITED STATES"
  - What is HT? Click on it to filter the data table by it.  In the locality column we see that it is Haiti.  Edit via the text facet to be HAITI. 
...
````
## Numeric Facet 
Numeric Facets create a histogram of values and allow you to filter the data table to immediately address odd values. 
